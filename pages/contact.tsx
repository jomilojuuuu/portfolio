    ...
    <div className='h-screen relative flex flex-col md:text-left md:flex-row max-w-7xl px-10 justify-evenly mx-auto items-center'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col space-y-2 w-fit mx-auto'>
        <div className='flex space-x-2'>
          <input {...register('name')} placeholder="Name" className='contactInput w-[155px] md:w-[310px]' type="text" />
          <input {...register('email')} placeholder="Email" className='contactInput w-[154px] md:w-[310px]' type="email" />
        </div>

        <input {...register('subject')} placeholder="Subject" className='contactInput w-[318px] md:w-[630px]' type="text" />
        <textarea placeholder="Message" className='contactInput w-[318px] md:w-[630px]'></textarea>
        <button type='submit' className='bg-[#F7AB0A] py-5 px-10 rounded-md text-black w-[320px] md:w-[630px] font-bold text-lg'>Submit</button>
      </form>
    </div>
  )
}
